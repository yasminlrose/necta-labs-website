...
const StockistPortalPage = () => {
  ...
  const handleTabChange = (id: TabId) => {
    setActiveTab(id);
    router.push(`/stockist-portal?tab=${id}`);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const handleSignOut = async () => {
    await signOut();
    router.push("/stockist");
  };
  ...
  if (!user || !isStockist) {
    return (
      <>
        ...
        <StockistAuthModal isOpen={authOpen} onClose={() => { setAuthOpen(false); if (!user || !isStockist) router.push("/stockist"); }} />
      </>
    );
  }
